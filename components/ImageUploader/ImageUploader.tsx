...
export default function ImageUploader({ page, gallery }: IProps) {
  ...
  const deleteImg = async (item: imageProps) => {
    const original = item.original.replace('https://storage.googleapis.com/marineraenelmundoflorida.appspot.com/', '')
    const thumbnail = item.thumbnail.replace('https://storage.googleapis.com/marineraenelmundoflorida.appspot.com/', '')
        ...
        {gallery?.map((img: Object, index: Number) => {
          return (
            ...
            <div key={img.id} className="my-5">
              ...
              <img key={index} src={img.thumbnail} alt='uploaded file' height={'100%'} width={'100%'} />
              {/* @ts-ignore */}
              <button onClick={() => deleteImg(img)} style={{ position: 'absolute', display: 'block' }}>
                delete
              </button>
            </div>
          )
